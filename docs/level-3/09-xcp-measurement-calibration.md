...
XCP's calibration mechanism (letting an engineer tweak a constant like a PID gain live, without reflashing) works because the calibration variables are placed in RAM at addresses known to both the ECU's build (via an A2L description file mapping symbol names to addresses) and the calibration tool — XCP's `SET_MTA`/`DOWNLOAD` commands are, at the silicon level, nothing more than direct memory writes to those RAM addresses over whatever transport (CAN, Ethernet, USB) is carrying XCP; there's no special calibration hardware, the "magic" is entirely in address-book bookkeeping (the A2L file) kept in sync with the actual linked memory layout.

Measurement (streaming live variable values back to a host tool at high rate) is where the underlying hardware genuinely matters: for tight-jitter measurement, XCP's DAQ (Data Acquisition) lists are typically serviced from a hardware timer interrupt (an FTM compare-match, tying directly back to the timer mechanism covered in the timers/PWM module) so that samples are read at a hardware-timed cadence rather than whenever a background task happens to run — sampling a variable from a lower-priority polling loop instead of a timer ISR reintroduces exactly the kind of scheduling jitter that makes a captured signal useless for tuning a fast control loop.

Because DOWNLOAD writes go straight to RAM with no bounds checking beyond what the ECU's own code enforces, writing a calibration constant that a running control loop is reading *concurrently* creates a genuine race condition at the memory level — if the calibration variable spans more bytes than the core's bus can write atomically, a control loop task could read a half-updated (torn) value mid-write; production calibration frameworks address this with double-buffered "page-switching" RAM regions specifically to avoid a torn read at the hardware memory-access level, not just as a software style preference.
